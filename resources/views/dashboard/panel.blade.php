...
<table align="center" height="auto" width="100%" border="0" cellspacing="1" cellpadding="1" id="tb_pro" class="dashboard-table {{ $weekCountClass }}">
	...
	<tr class="dashboard-table__row">
		...
		@foreach ($row['weekData'] as $weekIndex => $weekData)
			...
			<td align="center" class="cls_vals"><img src="{{ $metricIconUrl($weekData['icon']) }}" class="box" />{{ number_format($weekData['percent'], 0, ',', '') }}%</td>
		@endforeach
		...
		<td align="center" class="cls_vals cls_bk"><img src="{{ $metricIconUrl($row['totalIcon']) }}" class="box" />{{ number_format($row['totalPercent'], 0, ',', '') }}%</td>
	</tr>
	@endforeach
...
@endif
	...
	<tr>
		...
		@foreach ($summary['weekTotals'] as $weekTotal)
			...
			<td align="center" class="cls_vals2 cls_bk"><img src="{{ $metricIconUrl($weekTotal['icon']) }}" class="box" />&nbsp;<b>{{ number_format($weekTotal['percent'], 1, ',', '') }}%</b></td>
		@endforeach
		...
		<td align="center" class="cls_vals2 cls_bk"><img src="{{ $metricIconUrl($summary['grandIcon']) }}" class="box" />&nbsp;<b>{{ number_format($summary['grandPercent'], 1, ',', '') }}%</b></td>
	</tr>
	<tr height="5px"></tr>
...
<table align="center" height="6%" width="25%" border="1" cellspacing="3" cellpadding="3" id="tb_tot" class="dashboard-total-table">
	<tr>
		...
		<td align="center" class="dashboard-total-table__plain"><img src="{{ $metricIconUrl($summary['netIcon']) }}" class="box" />{{ number_format($summary['netPercent'], 1, ',', '') }}%</td>
	</tr>
</table>
<br><br><br><br>
@endif
