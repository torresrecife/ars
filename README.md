...
- `app/Http/Controllers`: HTTP entrypoints and administrative controllers.
...
Authentication uses Laravel `auth` and `guest` middleware. Authorization for administrative modules is handled through policies/gates by resource.

Known user levels:

- `ADM`: full administration.
