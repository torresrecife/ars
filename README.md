...
- administração de usuários, clientes, setores, regiões, semanas, andamentos e metas;
...
- `app/Http/Controllers`: entrypoints HTTP e controllers administrativos.
...
Endpoints administrativos REST/JSON:
...
A autenticação já usa fluxo Laravel com `auth` e `guest`. A autorização dos módulos administrativos usa policies/gates por recurso.
...
- policies nos módulos administrativos;
- endpoints REST/JSON para módulos administrativos;
- frontend administrativo migrando para client AJAX comum;
- views administrativas e operacionais em Blade;
...
- algumas views administrativas com inline styles residuais;
