...
if($my_event=='Faturamento'){
	...
	$querys .= " AND (p.NumeroContratoNeoCobranca=(SELECT top 1 pp.NumeroContratoNeoCobranca FROM Processo AS pp WITH (NOLOCK) WHERE pp.CodigoProcesso=l.CodigoProcesso ORDER BY pp.NumeroContratoNeoCobranca ASC ) OR p.NumeroContratoNeoCobranca IS NULL) ";
