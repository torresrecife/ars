...
function carregarFiltros(idTabela) {
    ...
    for(var i = 0; i < colunasComFiltro.length; i++) {
        ...
        var saida = "<select id=\"" + idColuna + "\" col=\""+i+"\" style=\"max-width:120px;color:#333;font-weight:bold; font-family:arial; font-size:9pt\">\n";
...
function filtrarColuna(idTabela) {
	...
	if ($('#id_crs:visible').is(":visible") == true) {
		...
		$("#id_crs").html("Valor Total: <span style='color:#333;font-family:arial'><b> R$ " + crs.formatMoney(2,',','.') + "</b></span>");		
	}
	$("#id_sel").html("Total selecionado: "+cls+"");
