				...
				if( (($m==2 || $m==5 || $m==8 || $m==11) && $ncol==14) ||
					...
					if( (($m==3 || $m==6 || $m==9 || $m==12) && $ncol==14) ||
						...
						if($m==$ncol){
							...
							$html_1 .= "<td align='center' class='cls_vals cls_bk' ><img src='http://***REMOVED***/img/$cor' class='box' />".$tot."%</td>";
							$codig_and=array();
						}else{
							$html_1 .= "<td align='center' class='cls_vals'><img src='http://***REMOVED***/img/$bol' class='box' />" . ($tt_meta!=0?number_format(($banq/($tt_meta))*100,0,',',''):0)."%</td>";
						}
					}
				}
			}
...
if($Wrow_2>0){
				...
				if( (($m==2 || $m==5 || $m==8 || $m==11) && $ncol==14) || 
					...
					if( (($m==3 || $m==6 || $m==9 || $m==12 ) && $ncol==14) ||
						...
						if($m==$ncol){
							...
							$html_2 .= "<td align='center' class='cls_vals cls_bk' ><img src='http://***REMOVED***/img/$cor_2' class='box' />".$tot_2."%</td>";
						$codig_lnc=array();
						}else{
							$html_2 .= "<td align='center' class='cls_vals'><img src='http://***REMOVED***/img/$bol' class='box' />".number_format(($tt_meta!=0?($Wbanc_qtd/($tt_meta)):0)*100,0,',','')."%</td>";
						}
					}
				}
			}
			...
			for($m=1;$m<=($ncol);$m++){
				if($m==1){
					...
					if($m==$ncol){
						...
						$html_2 .= "<td align='center' class='cls_vals2 cls_bk' ><img src='http://***REMOVED***/img/$Mbol' class='box' />&nbsp;<b>".$mt."%</td>";
					}else{
						if( (($m==2 || $m==5 || $m==8 || $m==11) && $ncol==14) || 
							(($m==2 || $m==5 || $m==8 || $m==11 || $m==14)  && $ncol==17)){
							$Tm = (${"M_".$m});
						...
						(($m==3 || $m==6 || $m==9 || $m==12 || $m==15)  && $ncol==17) ){
							...
							$html_2 .= "<td align='center' class='cls_vals2 cls_bk'><img src='http://***REMOVED***/img/$Tbol' class='box' />&nbsp;<b>".$Tp."%</b></td>";
						}
					}
				}
			}
	...
	$html_3 .= "<table align='center' height='6%' width='25%' border='1' cellspacing='3' cellpadding='3' id='tb_tot' style='font-family:arial;font-size:8pt; border-collapse: collapse;'>";
		...
		$html_3 .= "</td>";
		$html_3 .= "<td align='center' style='background:#ffffff'><img src='http://***REMOVED***/img/$cor_3' class='box' />";
		$html_3 .= $tot_3 . "%";
		$html_3 .= "</td>";
	$html_3 .= "</tr>";
	$html_3 .= "</table>";
