...
<table align='center' height='50%' width='99%' border='0' cellspacing='3' cellpadding='3' style='font-family:arial;font-size:8pt; border-collapse: collapse;background:#ffffff'>
	...
	<tr style='height:30px'>
		...
		<?php foreach ($row['weekData'] as $weekData): ?>
			...
			<td class='cls_body' align='center'><img src='http://10.81.11.202/img/<?php echo $weekData['icon']; ?>' class='box' /><?php echo number_format($weekData['percent'], 0, ',', ''); ?> %</td>
		<?php endforeach; ?>
		<td class=''>&nbsp;</td>
		...
		<td class='cls_body cls_bk' align='center' style='color:#000'><img src='http://10.81.11.202/img/<?php echo $row['totalIcon']; ?>' class='box' /><?php echo number_format($row['totalPercent'], 0, ',', ''); ?> %</td>
	</tr>
	<?php endforeach; ?>
	<tr height='5px'></tr>
	<tr>
		...
		<?php foreach ($totals['weeks'] as $weekTotal): ?>
			...
			<td align='center' class='cls_vals2 cls_bk'><img src='http://10.81.11.202/img/<?php echo $weekTotal['icon']; ?>' class='box' /><b><?php echo number_format($weekTotal['percent'], 0, ',', ''); ?> %</b></td>
		<?php endforeach; ?>
		<td class=''>&nbsp;</td>
		...
		<td align='center' class='cls_vals2 cls_bk'><img src='http://10.81.11.202/img/<?php echo $totals['icon']; ?>' class='box' /><b><?php echo number_format($totals['percent'], 0, ',', ''); ?> %</b></td>
	</tr>
