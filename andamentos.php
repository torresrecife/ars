...
<div>
<table class="***REMOVED***list" style="width:100%">
	<tr height="30">
...
while ($arr = mysqli_fetch_array($query)) {
	...
	<tr>
		...
		<td class="order"><?php echo htmlspecialchars($arr['chave'], ENT_QUOTES, 'UTF-8'); ?></td>
		<td class="order"><?php echo htmlspecialchars($arr['anda_neo'], ENT_QUOTES, 'UTF-8'); ?></td>
		<td class="order" style="color:#ffffff;background:<?php echo ($arr['especie'] == 1 ? "#1C86EE" : "#FFB90F"); ?>"><?php echo $Mtipo[$arr['especie']]; ?></td>
		<td class="order"><?php echo htmlspecialchars($arr['painel'], ENT_QUOTES, 'UTF-8'); ?></td>
		<td class="order"><?php echo htmlspecialchars($arr['titulo'], ENT_QUOTES, 'UTF-8'); ?></td>
...
<style>
.andamento-tipos-lista{
	margin:8px 0 0;
	width:100%;
	text-align:left;
}
.andamento-tipos-item{
	display:flex;
...
.andamento-tipos-vazio{
	margin-top:8px;
	font-size:9pt;
	color:#666;
}
.andamento-tipos-remover{
	margin-left:8px;
	padding:2px 8px;
	font-size:9pt;
	cursor:pointer;
