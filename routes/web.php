...
Route::middleware('legacy.auth')->group(function () {
    ...
    Route::match(['get', 'post'], '/admin', function (Request $request) {
        return app('App\Http\Controllers\HomeController')->webSectionPage($request, 'admin');
    })->name('admin');
    ...
    Route::match(['get', 'post'], '/metas', function (Request $request) {
        ...
        return app('App\Http\Controllers\HomeController')->webSectionPage($request, $hasMetaContext ? 'metas-admin' : 'metas-select');
    })->name('metas');
